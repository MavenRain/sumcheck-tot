...
MUTATIONS = [
    ("polynomial-agreement-bound-cardinality",
     """def ScPolynomialAgreementBound : (0 F : Type 0) -> ScFinite F ->
  ...
  fun F finite plus times d => (p : ScPolynomial F d) -> (q : ScPolynomial F d) ->
    ...
    ScLe (scFiniteCount F
      (fun r => Eq F (scPolynomialEval F plus times d p r)
        (scPolynomialEval F plus times d q r))
      (fun r => scDecEq F finite (scPolynomialEval F plus times d p r)
        (scPolynomialEval F plus times d q r)) finite) (scCardinality F finite)""", 1),
    ("polynomial-round-distinct-trivialized",
  ...
  fun F plus times lo hi d n p g claim w valid different equal => different
    ...
    (Eq F claim (scSum F plus lo hi (succ n) g) -> ScEmpty) ->
    Eq (ScPolynomial F d) p
      (scWitnessPolynomial F plus times d (scMarginal F plus lo hi n g) w) -> ScUnit :=
  fun F plus times lo hi d n p g claim w valid different equal => scUnit""", 1),
    ("scPolynomialRoundAgreement-trivialized",
  ...
  fun F finite plus times lo hi d n p g claim w agreement valid different => scLeTrans
    ...
    (Eq F claim (scSum F plus lo hi (succ n) g) -> ScEmpty) ->
    Eq Nat d d :=
  fun F finite plus times lo hi d n p g claim w agreement valid different => refl Nat d""", 1),
    ("scPolynomialAgreementTree-trivialized",
  ...
  | succ k => fun strategy => match strategy as s return
    ...
    | pair polynomial next => fun g claim target agreement => pair
     ...
     """def scPolynomialAgreementTree : (0 F : Type 0) -> (finite : ScFinite F) ->
    (plus : F -> F -> F) -> (times : F -> F -> F) ->
    (distribute : (x : F) -> (a : F) -> (b : F) ->
      Eq F (times x (plus a b)) (plus (times x a) (times x b))) ->
    (shuffle : (a : F) -> (b : F) -> (c : F) -> (e : F) ->
  ...
  fun F finite plus times distribute shuffle lo hi d n strategy g claim target agreement =>
    refl Nat d""", 1),
    ("scPolynomialConditionalSoundness-trivialized",
  ...
  fun F finite plus times distribute shuffle lo hi d n strategy g claim target agreement
    ...
    Eq Nat d d :=
  fun F finite plus times distribute shuffle lo hi d n strategy g claim target agreement
      different => refl Nat d""", 1),
    ("scPolynomialConditionalSoundnessScaled-trivialized",
  ...
  fun F finite plus times distribute shuffle lo hi d n strategy g claim target agreement
      different => refl Nat d""", 1),
    ("polynomial-agreement-tree-wrong-child",
     """          (next r) (scRestrict F g r) (scPolynomialEval F plus times d polynomial r)
""",
     """          (next lo) (scRestrict F g r) (scPolynomialEval F plus times d polynomial r)
""", 1),
    ("polynomial-agreement-tree-wrong-claim",
     """          (next r) (scRestrict F g r) (scPolynomialEval F plus times d polynomial r)
""",
     """          (next r) (scRestrict F g r) (scPolynomialEval F plus times d polynomial lo)
""", 1),
    ("polynomial-agreement-tree-wrong-target",
     """          (next r) (scRestrict F g r) (scPolynomialEval F plus times d polynomial r)
          (scPolynomialTargetRestrict F plus times d k g target r) agreement)""",
     """          (next r) (scRestrict F g r) (scPolynomialEval F plus times d polynomial r)
          (scPolynomialTargetRestrict F plus times d k g target lo) agreement)""", 1),
...
MUTATION_DIAGNOSTICS = {
    ...
    "scPolynomialAcceptingCountErase-trivialized": "not a function type: scunit",
